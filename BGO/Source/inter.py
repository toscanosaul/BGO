...
class Miscellaneous:
    def __init__(self,randomSeed,parallel,create=True,nF=0,tP=0,
                 ALG="SBO",prefix=""):
        """
        Args:
            -randomSeed: Random seed used to run the problem.
                Only needed for the name of the files with the results.
            -parallel: True if we want to run the multistart gradient
                ascent algorithm and train the kernel of the GP in parallel;
                otherwise, it's false. 
            -folder: Path where the files with the results are saved. 
            -create: True if we want to create new files for the results;
                     it's false otherwise. If we want to add more results
                     to our previos results, this variable should be false.
        ...
        tempDir="Results"+'%d'%nF+"AveragingSamples"+'%d'%tP+"TrainingPoints"
        nameDirectory=prefix+tempDir
...
class opt:
    def __init__(self,numberParallel,dimXsteepestVn=None,dimXsteepestAn=None,
        ...
        -numberParallel: Number of starting points for the multistart gradient
                         ascent algorithm.
        -dimXsteepestVn: Dimension of x when VOI is optimized.
            We may want to reduce the dimension of the original problem.
        -dimXsteepestAn: Dimension of x when a_{n} is optimized. We may want
            to reduce the dimension of the original problem.
        -transformationDomainXVn: Transforms the point x given by the steepest
            ascent method to the right domain of x.
            Its arugment is:
                -x: The point to be transformed
        -transformationDomainXAn: Transforms the point x given by the steepest
                ascent method to the right domain of x.
                Its arugment is:
                    -x: The point to be transformed
        -transformationDomainW: Transforms the point w given by the steepest
            ascent method to the right domain of w.
            Its argument is:
                -w: The point to be tranformed.
        -projectGradient: Project a point x to the domain of the problem at
            each step of the gradient ascent method if needed.
            Its argument is:
                -x: The point that is projected.
        -functionGradientAscentVn: Function used for the gradient ascent method.
            It evaluates the VOI, when grad and onlyGradient are False;
            it evaluates the VOI and computes its derivative when grad is True
            and onlyGradient is False, and computes only its gradient when
        ...
        -functionGradientAscentAn: Function used for the gradient ascent method.
            It evaluates a_{n}, when grad and onlyGradient are False;
            it evaluates a_{n} and computes its derivative when grad is True and
            onlyGradient is False, and computes only its gradient when gradient
        ...
        -functionConditionOpt: Gives the stopping rule for the steepest ascent
            method, e.g. the function could be the Euclidean norm.
            Its arguments is:
        ...
        -cons: Constraints of the problem if slsqp is used. See
            http://docs.scipy.org/doc/scipy-0.14.0/reference/tutorial/
                optimize.html#tutorial-sqlsp
...
class data:
    ...
    def getTrainingDataKG(self,trainingPoints,noisyG,numberSamplesForG,
                          parallel):
        Xtrain=self.Xhist
        yTrain=np.zeros([0,1])
    ...
    def getTrainingDataSBO(self,trainingPoints,noisyF,numberSamplesForF,
        ...
        if parallel:
            ...
            pool = mp.Pool()
            for i in xrange(trainingPoints):
                job = pool.apply_async(noisyF,(XWtrain[i:i+1,:],
                                               numberSamplesForF))
                jobs.append(job)
            pool.close()  
            pool.join()  
